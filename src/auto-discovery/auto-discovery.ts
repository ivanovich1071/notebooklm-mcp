...
export class AutoDiscovery {
  ...
  async discoverMetadata(
    notebookUrl: string,
    maxRetries = 2
  ): Promise<AutoGeneratedMetadata> {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        log.info(
          `🔍 Auto-discovering metadata (attempt ${attempt + 1}/${maxRetries + 1})...`
        );
        ...
        if (attempt === maxRetries) {
          log.error(
            `❌ Failed to auto-discover metadata after ${maxRetries + 1} attempts`
          );
          throw new Error(
        ...
        log.warning(
          `⚠️  Attempt ${attempt + 1} failed: ${errorMessage}. Retrying in 2s...`
        );
  ...
  private parseResponse(response: string): AutoGeneratedMetadata {
    try {
      ...
      if (!parsed.name || !parsed.description || !parsed.tags) {
        throw new Error(
          'Missing required fields (name, description, or tags)'
        );
      }
