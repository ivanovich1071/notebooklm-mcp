...
function directoryHasLocales(dir: string): boolean {
  return fs.existsSync(path.join(dir, 'fr.json')) && fs.existsSync(path.join(dir, 'en.json'));
}
...
function getI18nDir(): string {
  ...
  throw new Error(`Could not find i18n locale files. Checked: ${candidates.join(', ')}`);
}
